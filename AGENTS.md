...
- The speed cap binds before the law does at short range: `fps_dash` (burst 4×0.4 = 1.6 px/ms) and the parts of `fps_jump_3m` where |v_z| > 1.5 px/ms are saturated, so their error is a cap/plant property — check `arena.diag`'s `est_share` before attributing such errors to the control law, and see `-x` in the launch scripts.
