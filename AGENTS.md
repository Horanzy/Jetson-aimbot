...
| `src/io/pad_calib.cu/.h` | pad calibration, independent of the hid one (own trigger, own storage, own write-back, own state, own excitation tables — nothing of hid's `CalibSeg` tables or `run_calibration` is shared): L3+R3 held 5 s or the `padcalib` hot param; the state machine (start cross → per-axis per-level excitation with inter-segment zero pauses while `g_calib_collect` → settle → `g_calib_request`/`g_calib_done`/3 handshake → nod/shake, with a bounded replay ladder and a one-shot whole-round re-run) driven by the 1 kHz pad tick; **pause + mid-segment method** — the per-segment pauses give L (command edge → picture truly stops, two independent readings) and the noise floor σ, the segment mid-window gives each level's gain with no delay alignment, the level gains fit a power law whose full-deflection extrapolation is the axis's screen speed; the level set {30,40,50,60,70}% with the **symmetric segment order [+d,−d,−d,+d]** (travel centred on the level's start point, two measurements per direction), the validity gates (mid-window sample count ≥ 8 with a 4-frame jitter margin / significance vs σ / response / block-spread and per-frame aliasing range / direction-group consistency), the **texture-vs-clamp split** (a response-gate failure means that direction has no correlatable content → the other direction may be used alone, logged; a significance-gate failure means the picture does not move → the level is dropped), the degradation rule (≥2 levels → fit, exactly 1 → linear fallback, 0 → fail), the **equal-travel adaptive segment duration** (travel target = screen/8 = 135 px, bounded below by the mid-window statistics 258 ms@120fps and above by the 600 ms ceiling, corrected once per level by the previous level's measured travel), the **sampling probe** (loose 2σ criterion + magnitude clamp, driving the next level's duration), the replay ladder and the whole-round re-run, the design band whose edge means "measurement not credible → fail rather than write a clamped value", the dead-zone diagnostic (log only), and the `PAD_STICK_GAIN_X`/`PAD_STICK_GAIN_Y`/`L_EST_PAD` VAR names |
...
- **Mouse (hid)**: aim at a static background with texture, hold both side keys for 5 s. Start = draw a cross centred on the cursor's start point; success = nod; failure = shake. With `-S`, `S_EST=`/`L_EST=` are written back into the script automatically (atomic rename). The excitation is a **cross centred on the start point** (+x → −x → −x → +x → +y → −y → −y → +y, every leg rest-to-rest with a 150 ms zero-command pause between legs, 3 loops ⇒ 24 legs/9.6 s): the per-axis four-leg pattern makes the partial sums run 0 → +A → 0 → −A → 0, so the travel envelope is ±A around wherever the operator left the cursor (the old single-sided square required starting from a corner or the system cursor clamps to a screen edge and the camera stops answering), every loop returns to the start point (similar picture content → steadier block correlation) and each leg contributes two clean edges (16 per loop vs the square's 4) which sharpens the lag-alignment regression. Pauses do **not** fix a systematic underestimation — the reversal's aggregate window is already consistent — they dilute the reversal-adjacent samples (the only ones sensitive to the lag grid), sharpen the edges and give a static noise-floor estimate for the log. `run_calibration` itself is unchanged (its lag grid steps derive from `TICK_MS` and the observed frame interval).
- **Pad**: L3+R3 held 5 s, or the webui's 「开始标定」 button (hot param `padcalib=1`, consumed once — a request arriving while a run is in progress is logged and dropped, never re-entrant). The firmware owns the right stick for the whole run (human right-stick input is ignored, the law stands down), and the excitation is **per axis, per level, with a zero-deflection pause after every segment** (the start cross / nod / shake are at 30 % deflection — small and slow, no full-deflection swings): the level set is {30, 40, 50, 60, 70} % on both axes (the low end stops where the stick's dead zone starts — 10 % was measured to barely move the picture, which drags the extrapolation down — and 70 % is the highest level whose per-frame shift normally stays inside the correlation range, keeping the extrapolation span to full deflection at 1/0.7 = 1.43), each level played in the **symmetric order [+d, −d, −d, +d]** so the travel envelope is centred on the level's start point (±A, no need to start from a corner) and each direction is measured twice, every segment followed by a 150 ms pause. The segment duration is **equal-travel**: T = 135 px / V̂(next level), clamped to [258 ms@120fps, 600 ms], so slow games get longer segments, fast games are caught by the sample-count floor, and each level logs its expected travel and which clamp bound it (the measured travel of the previous level then corrects the next one by a half-damped ratio). If a level's finished segments have fewer than 8 mid-window samples it is **replayed with T ← min(2T, 600 ms)**, up to the ceiling (258/516/600 ms at 120 fps); if a whole round ends with an axis short of two levels *purely* because of sample shortage, the round is re-run once with the floor raised one rung (258→516 ms). Two quantities are then directly measurable, without coupling gain to delay:
...
| Sensitivity s | least-squares calibration (lag sweep: coarse step = the observed frame interval, fine step = `TICK_MS` = 1 ms, fine range ±1 coarse step) | hip-fire calibration | constant |
