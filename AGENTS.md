...
| `src/io/pad_output.cu/.h` | pad-mode merge layer: law velocity → right-stick injection conversion + human-channel merge clamp, stick ledger Σ(deflection·ms), `own_motion_ledger` mode routing, `--pad-dump` printing; the 1kHz pad tick (calibration step first — the excitation owns the right stick and the law stands down) publishes the merged logical state into `g_pad_publish` (latest-slot + seq — the output backend's polling contract; the XInput backend consumes it, never rewrites sticks); `g_pad_stick_gain` (runtime full-deflection screen speed, set by `-G` and by the pad calibration) is the single source of the injection conversion and of the pad speed cap |
| `src/io/pad_calib.cu/.h` | pad calibration, independent of the hid one (own trigger, own storage, own write-back, own state): L3+R3 held 5 s or the `padcalib` hot param; the state machine (start square → full-deflection square-wave excitation while `g_calib_collect` → settle → `g_calib_request`/`g_calib_done` handshake → nod/shake) driven by the 1 kHz pad tick; the excitation trajectory tables, the sensitivity → full-deflection-screen-speed conversion, the design band whose edge means "measurement not credible → fail rather than write a clamped value", and the `PAD_STICK_GAIN`/`L_EST_PAD` VAR names |
...
| `src/io/pad_test.cu` | `build/pad_test` unit test (built and run by `compile.sh`): injection/clamp math, ledger, trigger gating, ledger routing, absent-device non-blocking, publish-point contract, XInput wire format (report header, button bits, trigger mapping, int16 LE assembly and the Y convention) and device bytes; plus the pad calibration: excitation-table wall-clock/amplitude, trigger and phase progression (incl. re-trigger after a full cycle and unreachability under pure pass-through), stick ownership + ledger accounting, unit conversion + design band, the fit on a synthetic ledger/delay (both modes' branches, and the clamp only biting out of band), and the parameterized write-back (both VAR sets, atomic replacement, permission, append) |
...
-G full-deflection screen speed px/s (pad stick gain; default 3000, clamped to the design band,
   overwritten by a successful pad calibration)
...
- **Pad**: L3+R3 held 5 s, or the webui's 「开始标定」 button (hot param `padcalib=1`, consumed once — a request arriving while a run is in progress is logged and dropped, never re-entrant). The excitation is the same square as hid but at full deflection in each direction (the amplitude known before calibration — the stick gain is exactly what is being measured, just as the hid excitation's px speed is unknown until `s` is measured), with the hid design durations (240/250 ms edges, 300 ms settle); the firmware owns the right stick for the whole run (human right-stick input is ignored, the law stands down) and the measurement rides the same chain: `g_calib_collect` → block phase correlation → least squares + coarse/fine delay sweep, reading the stick ledger. The fit yields px per (deflection·ms); ×32767×1000 is the full-deflection screen speed. Success = nod (the values are live immediately and written back as `PAD_STICK_GAIN=`/`L_EST_PAD=`), failure = shake (nothing written). A fit landing outside the design band [300, 30000] px/s — a decade around the 3000 px/s design point; below the band the stick cannot track the physical requirement 2000 px/s by an order of magnitude, above it the per-frame screen shift is past the phase-correlation block's range — means the measurement is not credible and is reported as a failure rather than written back clamped (a screen that does not answer the injection, i.e. no game running, lands here). Buttons pass through unchanged during the run (as in hid): L3/R3 reach the game for the trigger's 5 s, so calibrate where holding them is harmless.
...
- One measurement chain for both modes: the state machine (hid in `core/control.cu`, pad in `io/pad_calib.cu`) plays the excitation and raises `g_calib_collect`; the capture thread samples phase-correlation displacements; on `g_calib_request` it fits `px = sensitivity × Δledger` (`run_calibration`) against `own_motion_ledger()` and writes the result back with the mode's VAR names. Only two things are mode-specific: which ledger the symbol resolves to, and the clamp band (px/count for hid, px per deflection·ms for pad, a decade around its design point). The law's bandwidth is untouched by calibration — it is derived from the calibrated `L` at tick time.
