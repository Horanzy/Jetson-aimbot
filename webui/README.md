...
- 标定值 `L_EST`（环路延迟，唯一标定量）**永远以脚本为权威**：每次启动现场解析；`-S` 指向 profile 脚本本身，
...
- **脚本 = 唯一事实源**: profile 参数就是 game 脚本头部的 `VAR=value` 块, 每次扫描现场解析,
  ...
  逐字保留, 执行位不变; `L_EST` 归固件标定回写管, WebUI 不碰（四个 `SPDX`/`SPDY`/`ADS_SPDX`/
  `ADS_SPDY` 速度倍率是手动项, WebUI 可改可热发, 固件不写）。SSH 改脚本 → 打开的
...
- 每次应用固件打印 `[热参] t=0.55`（UI 从日志流看到回执）；未知 key 忽略；启动时打印
  `✅ 热参数通道: 127.0.0.1:47700 (t/y/x/fov/spdx/spdy/adsspdx/adsspdy/k/aim/cap_*)` ——
