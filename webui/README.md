...
- 标定值 `S_EST`/`L_EST`（鼠标）与 `PAD_STICK_GAIN`/`L_EST_PAD`（手柄）**永远以脚本为权威**：
...
- **输出模式**（`OUTPUT_MODE` ↔ `output_mode`，冷参数）：`hid` = USB 鼠标（启动传 `-s`/`-l`），
  `pad` = XInput 手柄（启动传 `-M pad -P <子串> -G <满偏转屏速> -l <L_EST_PAD>`）；pad 组的
...
- **【◎ 开始标定】**（顶栏，仅 pad 模式运行中可用）：点按 → `POST /api/instance/padcalib` →
  ...
  「对准有细节的静止背景，按下后不要碰右摇杆」——固件之后独占右摇杆播放满偏方波（约 11 秒），
  成功 = 准星纵向点头两下并把 `PAD_STICK_GAIN`/`L_EST_PAD` 回写脚本，失败 = 横向摇头且不写。
  回执看日志 `[标定]` 行（`[标定] stick_gain=… px/s, L=… ms` = 回写成功；同一回执会进「标定」状态卡）。
...
13. 输出模式: 复制一份 profile → 输出模式改 `pad` →【保存】→ 脚本 `OUTPUT_MODE="pad"` 且
    `PAD_KEYWORD` 同批写入 → 【启动】: 启动命令预览出现 `-M pad -P …`（hid 模式则与改动前逐字相同），
    「手柄匹配子串」等 pad 参数组随之出现、「标定」卡片变成摇杆增益·L 形态;
14. 手柄标定: pad 实例运行中 → 顶栏【◎ 开始标定】可点（hid 实例/认领实例/旧固件时置灰或拒绝）→
    点按确认 → 日志出现 `[热参] padcalib=1` 与 `[标定] 触发`；成功则日志出现
    `[标定] stick_gain=… px/s, L=… ms` + `[标定] 已回写 …`，脚本 `PAD_STICK_GAIN`/`L_EST_PAD`
    被改写而 `S_EST`/`L_EST` 一字不动；失败（无游戏/画面不动）则只出现
    `[标定] 失败: …`，脚本不改。
