...
- 标定值 `S_EST`/`L_EST`（鼠标）与 `PAD_STICK_GAIN_X`/`PAD_STICK_GAIN_Y`/`L_EST_PAD`（手柄，逐轴满偏转屏速）**永远以脚本为权威**：
  每次启动现场解析（`calib` 字段，UI 只读显示）；`-S` 指向 profile 脚本本身，固件标定回写照旧
  落进脚本 —— SSH 侧与 UI 侧看到同一份标定。两套互不覆盖。
- **输出模式**（`OUTPUT_MODE` ↔ `output_mode`，冷参数）：`hid` = USB 鼠标（启动传 `-s`/`-l`），
  `pad` = XInput 手柄（启动传 `-M pad -P <子串> -G <x 轴屏速>,<y 轴屏速> -l <L_EST_PAD>`）；pad 组的
  「手柄匹配子串」只在 pad 模式显示。切换模式要重启进程（两模式互斥且各自独占 USB 设备控制器），
  所以它不是热参数 —— 保存后按【启动】生效。
- **脚本 = 唯一事实源**: profile 参数就是 game 脚本头部的 `VAR=value` 块, 每次扫描现场解析,
  没有独立参数存储。网页【保存】= 原子写回脚本 —— 只改目标变量的值, 行内注释/引号风格/其余行
  逐字保留, 执行位不变; `S_EST`/`L_EST` 与 `PAD_STICK_GAIN_X`/`_Y`/`L_EST_PAD` 归固件标定回写管, WebUI 不碰。SSH 改脚本 → 打开的
...
- **【◎ 开始标定】**（顶栏，仅 pad 模式运行中可用）：点按 → `POST /api/instance/padcalib` →
  向固件热参通道发一条 `padcalib=1`（fire-and-forget，与其它热参同一路径）。弹窗提示
  「对准有细节的静止背景，按下后不要碰右摇杆」——固件之后独占右摇杆播放小幅分级激励
  （逐轴 30/40/50/60/70% 逐级 + 段间停顿，慢游戏段长自动拉长，约 20 秒），成功 = 准星纵向点头两下并把
  `PAD_STICK_GAIN_X`/`PAD_STICK_GAIN_Y`/`L_EST_PAD` 回写脚本，失败 = 横向摇头且不写。
  回执看日志 `[标定]` 行（`[标定] stick_gain_x=… px/s, stick_gain_y=… px/s, L=… ms` = 回写成功；
  同一回执会进「标定」状态卡）。
  前置条件不满足时后端**如实拒绝**（实例未运行 / 认领实例 / 固件无热参 / 运行中的 profile 不是
  pad），不盲发。**鼠标标定没有按钮**：双侧键长按 5 秒，保持原样。
...
| `aim` | 接管开关 (0=纯透传, 不注入移动; 检测/采集照常) | `-a` | 0 / 1 |
| `cap_fire` | 开火截图源开关 | `-e` | 0 / 1 |
| `cap_det` | 检测截图源开关 | `-e` | 0 / 1 |
| `cap_auto` | 定时截图源开关 | `-e` | 0 / 1 |
| `padcalib` | 请求开始手柄标定 (1=请求, 0=无操作; 固件一次消费即清) | 无 CLI (等价 L3+R3 长按 5s) | 0 / 1 |

- 每次应用固件打印 `[热参] t=0.55`（UI 从日志流看到回执）；未知 key 忽略；启动时打印
  `✅ 热参数通道: 127.0.0.1:47700 (t/y/x/fov/k/aim/cap_*/padcalib)` —— WebUI 以该行为准判断固件能力，
...
13. 输出模式: 复制一份 profile → 输出模式改 `pad` →【保存】→ 脚本 `OUTPUT_MODE="pad"` 且
    `PAD_KEYWORD` 同批写入 → 【启动】: 启动命令预览出现 `-M pad -P … -G <x>,<y>`（hid 模式则与改动前
    逐字相同），「手柄匹配子串」等 pad 参数组随之出现、「标定」卡片变成摇杆增益 X·Y·L 形态;
14. 手柄标定: pad 实例运行中 → 顶栏【◎ 开始标定】可点（hid 实例/认领实例/旧固件时置灰或拒绝）→
    点按确认 → 日志出现 `[热参] padcalib=1` 与 `[标定] 触发`；成功则日志出现
    `[标定] stick_gain_x=… px/s, stick_gain_y=… px/s, L=… ms` + `[标定] 已回写 …`，脚本
    `PAD_STICK_GAIN_X`/`_Y`/`L_EST_PAD` 被改写而 `S_EST`/`L_EST` 一字不动；失败（无游戏/画面不动）
    则只出现 `[标定] 失败: …`，脚本不改。诊断行 `[标定] 逐级 X/Y: …` 与 `[标定] 停顿法: …`
    给出每级的 (偏转级, 估计屏速, 是否有效/丢弃原因) 与噪声底/延迟读数。
