...
- hid 不碰手柄，pad/p5g 不读鼠标（手柄插着时它的辅助鼠标接口也会匹配 `-D` 的空模式，
  ...
  它决定整条输出后端，是冷参数。
- 标定回写按模式分流：hid 写 `L_EST`，pad/p5g 写 `L_EST_PAD`，两条独立互不覆盖；
  `-l` 取本模式那条（脚本里两条都在，`L_EST_PAD` 只被手柄模式消费）。
...
- 参数面按组落在脚本 VAR 上（每组一个 VAR，`app/discover.py` 的 `SCRIPT_VARS` 是唯一映射表）：
  ...
  - **拉枪速度** = `SPDX`/`SPDY`（腰射）与 `ADS_SPDX`/`ADS_SPDY`（ADS 键按住期间），四个逐轴整数，
    `100` = 基线、越大越快、有意义带 `5..2000`（与固件的 `spdx`/`spdy`/`adsspdx`/`adsspdy`
    一一对应）。它们是页面里唯一「改了就立刻生效」的手感旋钮 —— 速度不标定，固件也从不写它们。
- 标定值（环路延迟）**永远以脚本为权威**：每次启动现场解析；`-S` 指向 profile 脚本本身，
  ...
  绝不写编造的值（屏幕完全静止/无响应/尾迹被停顿截断都会如实失败）。日志里 `[标定]` 行**照常
  进页面日志流**（逐级读数与结论都在这场运行里），只有 60s 一行的报告率观测
  （`[USB-HID]`/`[PAD-USB]`）与 `[SAVE]`/`[AI FPS]` 被页面流隐藏。
- **脚本 = 唯一事实源**: profile 参数就是 game 脚本头部的 `VAR=value` 块, 每次扫描现场解析,
  ...
  - `L_EST`/`L_EST_PAD` 归固件标定回写管, WebUI 不碰; 四个 `SPDX`/`SPDY`/`ADS_SPDX`/`ADS_SPDY`
    速度倍率是手动项, WebUI 可改可热发, 固件不写。
...
13. **输出模式**: 参数页「输出模式」选 pad/p5g → 出现「手柄匹配子串 -P / 手柄触发阈值 -T /
    合并态调试输出 --pad-dump」，hid → 只出现「鼠标匹配子串 -D」；【保存】后脚本对应行变化；
    「运行」页的「启动命令预览」里随之出现 `-M pad -P … -T 6`（hid 是 `-D …`），`-l` 的来源
    VAR 在 `L_EST_PAD` 与 `L_EST` 之间切换。
14. **标定按钮**: 跑着 pad/p5g 实例时「运行」页的「开始标定」可点 → 日志出现
    `[热参] padcalib=1 (请求手柄标定)` 与随后的 `[标定] …` 行；hid 模式下按钮置灰并写明
    触发方式是鼠标双侧键长按。
