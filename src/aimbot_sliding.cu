// ============================================================================
//  aimbot_sliding.cu — AI 视觉自瞄 (鼠标透传式) · 边界层滑模 + 弹道甩枪 (备选律, 鲁棒优先)
//
//  备选控制律 (纯自瞄, 无采集); 检测/标定/透传链路与主程序 aimbot.cu 一致。
//  本律由 arena 仿真 (arena/laws/sliding.py) 在统一多组场景下重调, 低补丁味 (参数由
//  原理导出): 边界层滑模收敛 + 延迟免疫弹道甩枪, 鲁棒优先 (主动牺牲匹配速度换鲁棒性)。
//  详见 AGENTS.md。
//
//  链路: 采集卡 (UVC 1080p NV12, -d 按名字选择) → GStreamer nvvidconv
//        → CUDA 预处理 → TensorRT YOLO 检测 → alpha-beta 目标跟踪 → 滑模控制律
//        → USB Gadget 透传
//
//  架构:
//   1. 目标跟踪 — alpha-beta 滤波器 (固定增益 PRED_ALPHA0/BETA0, 与 arena 律逐行一致;
//      本律不做 dt 归一) - 预测步扣除已知鼠标控制作用 (Smith, PRED_L_COMP=1.0 不过补偿)
//      - 测量跳变超门限 → 滤波器重置 (切目标/坏帧, 无 derivative kick)
//
//   2. 触发式在线标定 — 双侧键长按 5 秒, 全程自动
//
//   3. 控制律 — 边界层滑模 (鲁棒优先)
//      - 预测误差 ê = f + fvx·(age+L) − s·在途counts (Smith 去延迟, 不做过预测/前馈)
//      - 滑动面 σ=ê。到达相 (|σ|>制动距离): 开环 u=vmax·sign(σ) 甩枪。Lyapunov 到达条件
//        V̇≤−|σ|(vmax−|d|)<0 只要到达增益 vmax>扰动上界 D (目标最大速度) → 到达增益就是
//        物理速度上限 (非调参), 开环对延迟免疫。
//      - 收敛相 (|σ|≤制动距离): 边界层内线性 PI。收敛增益 Kp=wn=(90°−PM)π/180/L, PM=60°,
//        故意按完整延迟 L̂ 设计 (不依赖 Smith 精确对消) → 失配仍留相位裕度, 这是本律鲁棒
//        (而非快) 的根本。Ki=Kp²/(4ζ²), ζ=0.5 → Ki=wn² (积分转折=wn, 不侵蚀 PM 的最大积分)。
//      - 边界层厚度=制动距离=vmax·L (brake_factor=1.0): 一个量两重物理含义 (全速延迟内滑行
//        距离 + 陈旧反馈最小可稳定层)。到达/收敛在边界处速度 1.9:1 跳变用混合带平滑。
//      - I 项距离衰减 + 抗饱和 (条件积分 + 限幅)。
//
//  用户可调参数:
//      -x  最大速度 (px/s)  自瞄移动速度上限 (到达相用其 ×SLID_VMAX_FRAC)。
//  结构参数 (PM/ζ/制动/混合带) 为头部常量, 实机调参见 AGENTS.md。
// ============================================================================
...
const float PRED_ALPHA0   = 0.40f;                   // 位置增益 (固定, 与 arena/laws/sliding.py 逐行一致)
const float PRED_BETA0    = 0.18f;                   // 速度增益 (固定; 本律不做 dt 归一)
...
// ========================= 控制律: 边界层滑模收敛 + 弹道甩枪 (鲁棒优先) =========================
//  收敛增益按完整延迟 PM=60 兜底; 制动点=物理值 Vmax·L。详见 arena/laws/sliding.py 与 AGENTS.md。
const float SLID_VMAX_FRAC    = 0.95f;               // 到达相速度占 max_v 比例 (留 5% 防量化截断)
const float SLID_BRAKE_FACTOR = 1.0f;                // 制动距离 = FRAC×vmax·L (=最小稳定边界层); 不晚制动
const float SLID_PM_DEG       = 60.0f;               // 相位裕度: Kp=wn=(90°−PM)π/180/L (按完整延迟, 无量纲设计)
const float SLID_ZETA         = 0.5f;                // 收敛 PI 阻尼比: ζ=0.5 → Ki=wn² (积分转折=wn, PM 上限)
const float SLID_BLEND_FRAC   = 0.30f;               // 到达/收敛混合带 = FRAC×制动距离 (平滑 1.9:1 速度跳变)
const float SLID_I_GATE_FRAC  = 0.25f;               // 积分门限 = FRAC×制动距离 (边界处衰减到 20%, 防暂态饱和)
        ...
        if (found) {
            if (!filt_init) {
                ...
                if (std::hypot(inx, iny) > TRACK_JUMP_GATE) {
                    ...
                    float alpha = PRED_ALPHA0;       // sliding: 固定增益 (不做 dt 归一), 与 arena 律逐行一致
                    float beta  = PRED_BETA0;
        ...
        if (cal == 3) {
            ...
            if (aiming) {
                ...
                if (valid && age < TARGET_STALE_MS) {
                    ...
                    float vmax = max_v * SLID_VMAX_FRAC;
                    float L = std::max(1.0f, le);
                    float wn = (90.0f - SLID_PM_DEG) * 3.14159265358979f / 180.0f / L;
                    float kp = wn;                                       // 收敛增益 = 完整延迟 PM=60 线
                    float ki = kp*kp / (4.0f * SLID_ZETA * SLID_ZETA);   // ζ=0.5 → Ki=wn²
                    float brake_dist = vmax * L * SLID_BRAKE_FACTOR;     // = 物理制动距离 = 最小稳定边界层
                    float boundary_layer = std::max(1.0f, SLID_BLEND_FRAC * brake_dist);
                    float i_gate = SLID_I_GATE_FRAC * brake_dist;

                    float vcx, vcy;
                    if (r > brake_dist) {
                        // 到达相: 开环甩枪 (Lyapunov 到达增益=vmax>扰动上界), 边界层内混合
                        float v_inner = wn * brake_dist;
                        float speed;
                        if (r > brake_dist + boundary_layer) {
                            speed = vmax;
                        } else {
                            float blend = (r - brake_dist) / boundary_layer;
                            speed = v_inner + blend * (vmax - v_inner);
                        }
                        float scale = speed / r;
                        vcx = ex * scale;
                        vcy = ey * scale;
