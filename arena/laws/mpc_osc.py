"""arena/laws/mpc_osc.py — MPC + 振荡签名前馈门 (mpc 的失配加固版)。

在 mpc (滚动时域 QP, 原理 [1]-[5] 沿用其 docstring 编号) 之上新增一个
**失配自激防护**: 估计器/QP/时域/权重全部逐字节同 mpc; 唯一变更是当
检测到"创新振荡签名"时暂时隔离幽灵速度通路。本 docstring 只写新增
机制; mpc 部分 (PM/qf/N/L_inflate/α/β 约定) 见 arena/laws/mpc.py。

================================================================================
为什么需要这个门 (失效模式, trace 实测)
================================================================================
Smith 预测的在途扣除用相信窗 [t−L̂−dt, t−L̂]。标定残差 |L真−L̂| 大时
(宽延迟带两端: L_true=20 → ΔL=−35ms; L_true=80 → +25ms), 两窗 counts 差
≈ s·(自身指令变化率)·ΔL, 作为伪创新经 α-β 滤波的 β 通道泵入 fvx →
幽灵速度。幽灵经两条路回到指令: (a) QP 前馈 v0=vff·fvx (增益 1.0 全通),
(b) Smith 外推 e0 += fvx·T (T≈age+L̂, QP 增益 k·T≈0.5)。环路增益在
|ΔL| 大时 >1 → 自激极限环 → 永不进 3px 带 → composite=inf (原 mpc 宽延迟
L_true=20 实测: 末值 21px, 幅度 28px, ex 过零 10 次/500ms)。匹配时
ΔL=−5ms 垃圾小, 无自激 —— 所以修复必须"匹配时不可见"。

================================================================================
判别原理: 自激的唯一签名是"创新的交替", 不是"矛盾幅度"
================================================================================
  - 自激极限环: 伪创新由自身指令经错位窗产生, 环路振荡 → 创新大幅
    **交替翻转** (实测 L20 周期 ~100ms)。
  - 真实目标事件 (急停/反转/机动): 矛盾**单极性持续**到 v̂ 重建完成,
    之后创新与 v̂ 同向 —— 孤立反转/过冲在创新维度不构成快速交替。
  - s 失配追赶偏置: (s真−s信)·自身速率 恒定符号, 不翻转 → s 带零改动。
  - 慢速真实欠阻尼响应 (机动反转+过冲+追摆): 其振荡在 **误差 e0 维度**
    清晰可见, 但在**创新维度**被滤波 α 通道跟踪掉 (创新单极性、小), 与
    自激的创新维度签名可分。对比实验 (e0 维度翻转计数) 证实: e0 翻转
    无法区分"真实事件振铃"与"自激", 会误伤机动 (rmse +37%) — 故本律
    坚持在创新维度检测。

机制 (三个部件, 全部 σ/时间标定化, 无绝对 px 常数):
  1) σ̂ 在线自标定 + Huber 守卫: 创新方差 EMA (β 同滤波), 单帧创新
     > IN_SIGMA·σ̂ (=3σ) 视为信号不回训 —— 否则自激垃圾创新被 σ̂ EMA
     吸收 (实测 σ̂ 1→44px), 所有 σ 归一门限失明 (σ̂ 致盲机制)。
  2) 显著创新符号交替计数 (逐轴): |in| > IN_SIGMA·σ̂ 才计显著; 符号
     相对上次显著创新翻转、且距上次 ≤ W 才计一次翻转; flip 按 τ=W
     泄漏。W = L̂ + 2·τ_v: 相邻矛盾必须经由环路往返 (L̂) 加一次 v̂ 重建
     (τ_v = DT0/β₀, 帧率归一 —— dt/β 与帧率无关) 才可能同源; 真实反转
     事件间隔 ≥ 重建+追赶 ≫ W。JUMP_GATE 复位时清零 (目标切换非振荡)。
  3) 信任度门控 (ff_pi w_state 语义): flip ≥ FLIP_N (=2; 一次振荡至少
     两个方向反转; 孤立反转/过冲只 1 次) → w 升起 (~2 帧, τ=2·DT0),
     证据消失按标定 L 尺度恢复 (τ=cfg.L)。确认时**同时切断两条幽灵
     回路**: QP 前馈 v0 与 Smith 外推 fvx·T 都乘 ff_gate = 1−w
     (只切 v0 时外推回路单独残余 3px 振荡, 实测)。满信任 w=0 →
     ff_gate=1 → 与 mpc 逐字节一致。

效果 (本机 seeds 1,2,3 实测): matched 全行逐字节同 mpc (160.34);
宽延迟 L20: inf → 121.39 (有限), L30: 93.76 → 80.56 (−14%);
L40-70 与 s 带 0.70-1.30 全档与 mpc 相同 (±0.2% 内); worst mismatch
113.38、relock 444ms、fpsΔ 4.0%、OVERALL 144.81 全部不变;
fps_eval 两档 (clean/flaky) 各指标与 mpc 相同 (±0.1%)。

已知边界: L_true=80 (+25ms) 的慢速边缘气泡 (±6-13px, ~330ms 周期)
创新不可见 (被 α 通道跟踪), 本门不覆盖 → L80 仍 settle-fail。其自激
直接通路 (vff=1) 增益≈1, 压低它只能牺牲 matched 带宽或 FF 精确性
(对比实验实测全部超帕累托闸门) —— 按系统哲学交给标定, 不硬凑。

与 rejected paths 的边界: 不重瞄准 FF (只按振荡证据暂时隔离); 不做
常开机动自适应估计 (无第二模型; σ̂ 有 Huber 守卫, 无致盲路径); 不在线
适配延迟 (L̂ 恒为标定值, 翻转窗只做事件定时, 不回改 L̂)。

================================================================================
参数表 (新增参数; mpc 部分参数见 mpc.py)
================================================================================
参数        默认    依据                                          真机调参方向
----------  ------  --------------------------------------------  ----------------
IN_SIGMA    3.0     无量纲 (σ̂ 倍数, 同 ff_pi CUSUM_C 家族): 创新   假告警→↑;
                    超此倍数才算"显著", 低于即噪声不参与翻转计数。  自激漏检→↓。
FLIP_N      2.0     无量纲: 一次振荡 ≥2 次方向反转; 孤立反转/过冲   误伤机动→↑;
                    只有 1 次。升级阈值。                           自激压不住→↓。
w 升级/恢复  2·DT0  时间常数: 升级 ~2 帧 (ff_pi w_state 约定),     同 ff_pi。
            /cfg.L  恢复按标定 L 尺度。
W (=L̂+2τ_v) 导出    翻转窗: 环路往返 (L̂) + 2 次 v̂ 重建 (τ_v=DT0/β₀, 保持导出。
                    帧率归一)。真机上随标定 L 与 β₀ 自动伸缩。
...
class MpcOscLaw(Law):
    ...
    Q_NORM = 1.0              # 误差权重归一化 (只有 q/r 比值有物理意义, mpc docstring [1])
    JUMP_GATE = 100.0
    STALE = 200.0
    IN_SIGMA = 3.0            # 显著创新门限 (σ̂ 倍数, 同 ff_pi CUSUM_C 家族)
    ...
    def step(self, t: float, obs: Optional[Observation]) -> Tuple[int, int]:
        ...
        if self.filt and age < self.STALE:
            ...
            # 证据消失按标定 L 尺度恢复 (τ=cfg.L, 同 ff_pi w_state 语义)。
