...
(ballistic 两段结构的收敛段方法升级; 继承 ballistic 的弹道段与 ballistic/ff_pi
的已验证安全机制)。
...
FF 的 ghost-v̂ 防护 (三层, 辖区不相交):
   ...
   3. CUSUM 方向矛盾归零 (继承 ff_pi 已验证机制): 双向 CUSUM 只累计与 v̂
...
会在 L_true=30 制造稳定悬崖, 继承 ballistic 的结论)。α0=0.70: 幻影稳态误差
...
| CUSUM K/C/H  | .5/3/9σ | 继承 ff_pi: σ 倍数, 无量纲 (K 漂移/C 单帧封顶/H 告警)      |
