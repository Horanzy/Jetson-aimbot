...
--------------------------------------------------------------------------------
被控对象 (去延迟后) 是积分器 e_dot = −u。连续 LQR 代价 ∫(q·e² + r·u²)dt 对标量积分器给出
...
验算: L_hat=55 时 r=(330/π)²≈11036, k=√(1/11036)=0.00952,
wn·L_hat=0.524rad=30° → PM=60° 精确成立。
...
--------------------------------------------------------------------------------
Smith 预测器已把延迟从 QP 初态里扣掉 (QP 对象模型去延迟), 但有限时域仍须"看见"主导时间尺度。
...
走 FISTA 40 迭代。本机实测 eval ~3.6s、integrate 全程 ~7.5s; 嵌入式 500Hz 需评估,
